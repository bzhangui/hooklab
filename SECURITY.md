...
- The PostgreSQL mode offers an optional shared hourly accepted-event cap per tenant, enforced transactionally in PostgreSQL. It is disabled by default and is not a general cross-instance request/byte limiter. Externally managed identity, database-level row security, and automatic migration from SQLite remain absent. Consumer delivery is at least once; use the delivery ID as a deduplication key.
...
- The CI-only Compose override enables a localhost HTTP receiver to verify signature and retry behavior. Never use that override or `HOOKLAB_ALLOW_LOOPBACK_ENDPOINTS=1` for a public deployment. Backup manifests contain data counts and a one-way key fingerprint; keep them private alongside the archives, and use an independent encrypted store for the actual key.
